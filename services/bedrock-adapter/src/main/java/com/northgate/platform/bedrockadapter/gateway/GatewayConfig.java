package com.northgate.platform.bedrockadapter.gateway;

import com.northgate.platform.bedrockadapter.fixture.FixtureBedrock;
 ...
 *
 * northgate.bedrock.transport:
 *   mq        JMS (IBM MQ, or Artemis under the local-artemis profile)
 *   http      the bedrock-core-mock REST queue facade at northgate.bedrock.core-url. Laptops and
...
public class GatewayConfig {
    ...
    public BedrockGateway bedrockGateway(JmsTemplate jmsTemplate,
                                         ...
                                         @Value("${northgate.bedrock.transport:mq}") String transport,
                                         @Value("${northgate.bedrock.core-url:http://localhost:4600}") String coreUrl,
                                         @Value("${northgate.mq.receive-timeout-ms:5000}") long receiveTimeoutMs,
                                         @Value("${northgate.bedrock.request-queue:BEDROCK.REQ}") String req,
                                         @Value("${northgate.bedrock.reply-queue:BEDROCK.RESP}") String resp) {
        BedrockGateway mq;
