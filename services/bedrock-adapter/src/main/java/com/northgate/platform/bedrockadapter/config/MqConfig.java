package com.northgate.platform.bedrockadapter.config;
...
public class MqConfig {
    ...
    public ConnectionFactory ibmMqConnectionFactory(
        @Value("${northgate.mq.host}") String host,
        @Value("${northgate.mq.port}") int port,
        @Value("${northgate.mq.queue-manager:QM1}") String queueManager,
        @Value("${northgate.mq.channel:DEV.APP.SVRCONN}") String channel,
        @Value("${northgate.mq.user:app}") String user,
        @Value("${northgate.mq.password:}") String password) throws JMSException {
    ...
    public ConnectionFactory artemisConnectionFactory(
        @Value("${northgate.mq.artemis-url:tcp://localhost:61616}") String url,
        @Value("${northgate.mq.user:artemis}") String user,
        @Value("${northgate.mq.password:}") String password) {
        return new ActiveMQConnectionFactory(url, user, password);
    }

    @Bean
    public JmsTemplate jmsTemplate(ConnectionFactory connectionFactory,
                                   @Value("${northgate.mq.receive-timeout-ms:5000}") long receiveTimeout) {
