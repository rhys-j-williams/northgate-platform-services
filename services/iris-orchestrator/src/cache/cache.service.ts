 ...
 * a Redis restart during the 2024.06 train took retail login down for eleven minutes (INC0049930). Copied from bff-retail verbatim, PLAT-1902 wants it in a shared package.
 * The in process map is per pod and is NOT shared, so with two pods and no Redis a conversation
 * can lose its state mid chat. Locally there is one pod. Do not run it that way anywhere else.
 */
@Injectable()
