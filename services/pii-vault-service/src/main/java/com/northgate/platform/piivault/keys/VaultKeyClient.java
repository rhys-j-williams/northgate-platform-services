package com.northgate.platform.piivault.keys;
 ...
 * active (see {@code northgate.vault.allow-dev-key}). GIS signed this off for non-prod only,
 * GIS-0522.
 */
@Component
public class VaultKeyClient {
    ...
    public VaultKeyClient(RestTemplateBuilder builder,
                          @Value("${northgate.vault.url:http://localhost:4605}") String baseUrl,
                          @Value("${northgate.vault.token:CHANGEME-vault-token}") String token,
                          @Value("${northgate.vault.key-name:pii-fpe}") String keyName,
                          @Value("${northgate.vault.allow-dev-key:true}") boolean allowDevKey) {
    ...
    private static byte[] devKey(int version) {
        try {
            ...
            return MessageDigest.getInstance("SHA-256")
                .digest(("northgate-pii-dev-key-v" + version).getBytes(StandardCharsets.UTF_8));
