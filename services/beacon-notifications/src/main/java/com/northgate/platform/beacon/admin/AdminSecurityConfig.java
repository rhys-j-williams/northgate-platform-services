...
public class AdminSecurityConfig {
    ...
    public InMemoryDirectoryServer embeddedLdap(@Value("${northgate.ldap.embedded-port:0}") int port)
        throws LDAPException, IOException {
        InMemoryDirectoryServerConfig config = new InMemoryDirectoryServerConfig("dc=northgate,dc=local");
    ...
    public LdapAuthenticationProvider ldapAuthenticationProvider(InMemoryDirectoryServer ldap) {
        String url = "ldap://localhost:" + ldap.getListenPort() + "/dc=northgate,dc=local";
