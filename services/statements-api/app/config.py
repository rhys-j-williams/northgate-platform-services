...
class Settings:
    ...
    # Fixture bundle exported from @northgate/domain-fixtures. Same file the Java services read.
    fixtures_path: Path = Path(
        _env("NORTHGATE_FIXTURES", str(Path(__file__).resolve().parents[3] / "fixtures" / "northgate-fixtures.json"))
    )
    fixture_fallback: bool = _env("NORTHGATE_FIXTURE_FALLBACK", "true") == "true"
    # Bank legal name and address for the statement header. Marketing owns the wording (MKT-0230).
    bank_name: str = "Northgate Trust Bank, N.A."
    ...
    producer: str = "northgate statements-api"
